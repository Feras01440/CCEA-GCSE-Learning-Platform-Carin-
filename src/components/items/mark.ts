...
import { markOrder, stepsAsOrder } from "./order-marking";
...
export function matchesCommonError(raw: string, error: CommonError, pairNames?: readonly [string, string], variables?: readonly string[]): boolean {
  ...
  try {
    switch (p.kind) {
      ...
      case "algebraic":
        ...
        return checkAlgebraic(raw, { answer: p.latex, mode: "equivalent", variables: variables ? [...variables] : undefined }).correct;
      case "matrix": {
 ...
 */
function withCommonError(base: MarkResult, raw: string, opts: MarkOptions, lead?: string, pairNames?: readonly [string, string], variables?: readonly string[]): MarkResult {
  const hit = (opts.commonErrors ?? []).find((e) => matchesCommonError(raw, e, pairNames, variables));
  if (!hit) return base;
...
export function markAnswer(raw: string, spec: AnswerSpec, opts: MarkOptions = {}): MarkResult {
  ...
  switch (spec.kind) {
    case "numeric": {
      const v = checkNumeric(trimmed, toNumericSpec(spec, { accuracyInstructed: instructsAccuracy(opts.prompt) }));
    ...
    case "algebraic": {
      ...
      return v.correct ? base : withCommonError(base, trimmed, opts, undefined, undefined, spec.variables);
    }
    case "mcq": {
    ...
    case "table": {
      ...
      const v = checkTable(trimmed, spec, { accuracyInstructed: instructsAccuracy(opts.prompt) });
