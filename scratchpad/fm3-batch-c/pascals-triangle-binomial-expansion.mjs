...
import { pTriangle, pPowers, pGrid } from "./phone.mjs";
...
const blocks = [
  ...
  },
  {
    ...
    svg: figHero,
    caption: "Each number is the sum of the two above it; the lines show two of the sums.",
  },
  {
    type: "gate",
    id: "g1",
  ...
  { type: "h", text: "The powers fall and rise", role: "variant" },
  {
    ...
    svg: figStrip,
    caption: "Read across: the power of p falls from 4 to 0 as the power of q rises from 0 to 4.",
  },
  {
    type: "p",
...
{
  const fig = pGrid({ rows: ROWS, upTo: 6, fill: [7, 8], title: "Pascal's triangle written out to row 6, with an empty grid underneath: Row 1 has 8 boxes for row 7 and Row 2 has 9 boxes for row 8" });
  const cells = [];
  ...
  questions.push(
    qq("0001", {
      ...
      parts: [
        {
          id: "main",
          stem: "Complete rows 7 and 8 of Pascal's triangle in the grid. Row 1 of the grid is row 7 of the triangle and Row 2 is row 8.",
          marks: 2,
...
questions.push(
  qq("0002", {
    ...
    parts: [
      {
        id: "main",
        stem: "Use Pascal's triangle to write down $(p + q)^{5}$ in expanded form.",
        marks: 1,
        answer: expansion(5),
...
questions.push(
  qq("0003", {
    ...
    parts: [
      {
        id: "main",
        stem: "Use Pascal's triangle to write down $(p + q)^{7}$ in expanded form.",
        marks: 2,
        answer: expansion(7),
        scheme: [
...
{
  const fig = pGrid({ rows: ROWS, upTo: 5, fill: [6], title: "Pascal's triangle written out from row 0 to row 5, then an empty grid of seven boxes, Row 1, for row 6" });
  questions.push(
    qq("0007", {
      style: "exam-style",
      difficulty: 2,
      ...
      figures: [figure(fig, "Pascal's triangle from row 0 to row 5, then one row of seven empty boxes, labelled Row 1 (row 6), under the column numbers 1 to 7.")],
      parts: [
        {
          id: "a",
...
const verification = [
  verLog(note.id, {
    ...
    "command-words": CMD,
    tariff: TAR,
    "maths-symbolic": NUM("Every row, expansion and coefficient in the note and its three figures"),
