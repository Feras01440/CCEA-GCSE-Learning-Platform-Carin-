...
import { big, bFixed, bMul, bPow, bValue, figure } from "./lib.mjs";
import { ROWS, d4, exactDec, rat, terminatesWithin } from "./stat.mjs";
import { ladderSvg, conditionsSvg, labelCardSvg, anatomySvg, bracketCardSvg, tableSvg, pascalGridSvg, sup, shown } from "./binfig.mjs";
...
const figLadder = ladderSvg(T, {
  ...
  caption: "Each row is one term of (p + q)⁴ with p = 0.8 and q = 0.2.",
});
const figLadderShaded = ladderSvg(T, {
  successWord: "scores",
  failureWord: "misses",
  successOne: "score",
  failureOne: "miss",
  shade: [2, 4],
  routes: true,
  title: `The free-throw table with the rows for 2, 3 and 4 scores shaded, and two routes to their total, ${e(T.atLeast(2))}`,
});
const figConditions = conditionsSvg({
  rows: [
    ["A fixed number of trials", "n = 6 rolls, decided before the first roll"],
    ["Two outcomes on each trial", "a six (success) or not a six (failure)"],
    ["The same probability every time", "p = 1/6 on every roll"],
    ["Independent trials", "one roll does not change the next"],
  ],
  title: "The four checks for a binomial model, each met by rolling a fair dice 6 times and counting the sixes",
});
const figLabel = labelCardSvg({
  lines: [
    { label: "success", value: "the bus is late" },
    { label: "p = P(success)", value: BU.pStr },
    { label: "q = 1 − p", value: `1 − ${BU.pStr} = ${BU.qStr}` },
    { label: "n = trials", value: `${BU.n} school days` },
  ],
  title: `The label card for the bus question: success is late, p = ${BU.pStr}, q = ${BU.qStr}, n = ${BU.n}`,
  caption: "The stated 85% is q: being on time is not what is counted.",
});
const figLabelBlank = labelCardSvg({
  lines: [{ label: "success" }, { label: "p = P(success)" }, { label: "q = 1 − p" }, { label: "n = trials" }],
  title: "A blank label card with four lines to fill in: success, p, q and n",
});
const figAnatomy = anatomySvg({
  pieces: [
    { text: String(BU.coef(2)), note: ["coefficient:", "row 5 of Pascal's", "triangle"] },
    { text: "×" },
    { text: `${BU.pStr}${sup(2)}`, note: ["p to the power 2:", "2 late days"] },
    { text: "×" },
    { text: `${BU.qStr}${sup(3)}`, note: ["q to the power 3:", "3 on-time days"] },
  ],
  footer: "2 + 3 = 5 days: the two powers add up to n",
...
const figBracket = bracketCardSvg({
  right: `1 − (P(0) + P(1)) = 1 − (${e(T.term(0))} + ${e(T.term(1))}) = ${e(T.atLeast(2))}`,
  rightNote: "the bracket takes both unwanted terms off",
  wrong: `1 − P(0) + P(1) = 1 − ${e(T.term(0))} + ${e(T.term(1))} = ${re("note#bracket", M.bracket)}`,
  wrongNote: "only P(0) comes off; P(1) is added back on",
  title: "Two lines for at least 2 of Aoife's 4 free throws: with the bracket, and with the bracket left out",
  caption: "Write '1 − (' first, then fill the bracket.",
});
const figPhrases = tableSvg({
  widths: [124, 150, 196],
  rows: [
    ["the question says", "successes it means", "shorter route"],
    ["exactly 2", "2", "P(2)"],
    ["at least 2", "2, 3, 4, 5, 6", "1 − (P(0) + P(1))"],
    ["at most 2", "0, 1, 2", "P(0) + P(1) + P(2)"],
    ["more than 2", "3, 4, 5, 6", "1 − (P(0) + P(1) + P(2))"],
    ["fewer than 2", "0, 1", "P(0) + P(1)"],
  ],
  title: "What five phrases mean in 6 trials with the number 2, and the shorter route for each",
  caption: "Six trials, and the number 2 in each phrase.",
});
...
const blocks = [
  ...
  { type: "h", text: "Four throws, five outcomes" },
  {
    type: "p",
    md: "Aoife scores 80% of her free throws, and one throw does not affect the next. She takes four. She could score 0, 1, 2, 3 or all 4 of them, so there are five outcomes to put a probability on.\nThe table lists all five. Each row is one term of $(p + q)^{4}$, with $p = 0.8$ for a score and $q = 0.2$ for a miss.",
  },
  {
    type: "figure",
    alt: `A table with a row for each number of scores from 0 to 4. Each row gives the term, such as 4 × 0.8³ × 0.2 for 3 scores, its probability, and the outcome in words. The probabilities are ${e(T.term(0))}, ${e(T.term(1))}, ${e(T.term(2))}, ${e(T.term(3))} and ${e(T.term(4))}, and the last row shows they total 1.`,
    svg: figLadder,
    caption: "The coefficients 1, 4, 6, 4, 1 are row 4 of Pascal's triangle. The five probabilities add up to exactly 1, because one of the five outcomes must happen.",
  },
  {
    type: "gate",
    id: "g1",
  ...
  { type: "h", text: "1. When the binomial model fits" },
  {
    type: "p",
    md: "The terms of $(p + q)^{n}$ are probabilities only when four things are true.\nThere is a **fixed number of trials**, $n$.\nEach trial has **two outcomes**, called success and failure.\nThe **probability of success is the same** on every trial.\nThe trials are **independent**: one result does not change the next.\nCheck a situation against all four before you use the method.",
  },
  {
    type: "figure",
    alt: "Four boxes, each ticked. A fixed number of trials: n = 6 rolls. Two outcomes: a six or not a six. The same probability every time: p = 1/6. Independent trials: one roll does not change the next.",
    svg: figConditions,
    caption: "Rolling a fair dice 6 times and counting the sixes passes all four checks.",
  },
  {
    type: "gate",
    id: "g2",
  ...
  { type: "h", text: "2. Name the success first" },
  {
    type: "p",
    md: `The success is whatever the question counts, even when it sounds like bad news. Asked how likely a bus is to be late on exactly 2 of 5 school days, the success is **late**.\nQuestions often state the probability of the other outcome. If the bus is on time on 85% of days, then $p = ${BU.pStr}$ and $q = ${BU.qStr}$. Fill in four lines before any arithmetic.`,
  },
  {
    type: "figure",
    alt: `A label card with four lines filled in: success, the bus is late; p = ${BU.pStr}; q = 1 − ${BU.pStr} = ${BU.qStr}; n = ${BU.n} school days.`,
    svg: figLabel,
    caption: "The card for the bus. The 85% in the question belongs to q.",
  },
  {
    type: "callout",
    kind: "examiner",
    title: "The swap examiners keep reporting",
    md: "Writing the probability of the other outcome in for $p$ is the slip named most often in this question: 0.3 and 0.7 mixed in one year, 0.2 and 0.8 in two others. The four lines on the card stop it before it starts.",
    source: q2022,
  },
  {
    type: "gate",
    id: "g3",
    kind: "number",
    prompt: "Fill in the label card for this: a phone battery passes a quality check with probability 0.96, and a technician counts how many of 5 batteries fail. What is $p$?",
    answer: "0.04",
    explain: "The technician counts failures, so a failed battery is the success here: $p = 1 - 0.96 = 0.04$, $q = 0.96$ and $n = 5$.",
  },

  { type: "h", text: "3. Exactly r successes is one term" },
  {
    type: "p",
  ...
  {
    type: "figure",
    alt: `The term ${BU.coef(2)} × ${BU.pStr}² × ${BU.qStr}³ with a label under each piece: ${BU.coef(2)} is the coefficient from row 5 of Pascal's triangle, ${BU.pStr}² is p to the power 2 for 2 late days, ${BU.qStr}³ is q to the power 3 for 3 on-time days. A note says 2 + 3 = 5, so the powers add up to n.`,
    svg: figAnatomy,
    caption: "Every exactly-r term has these three pieces.",
  },
  {
    type: "callout",
    kind: "why",
  ...
  { type: "h", text: "4. Worked example: the lines the marks are for" },
  {
    type: "p",
    md: `**Question.** A bus is on time on 85% of school days, independently. Find the probability that it is late on exactly 2 of the 5 days from Monday to Friday. ${FOUR_DP}\n**Line 1.** Late is the success: $p = ${BU.pStr}$, $q = ${BU.qStr}$, $n = ${BU.n}$.\n**Line 2.** $P(\\text{exactly 2 late}) = ${symTerm(5, 2)}$. The method marks are for this line.\n**Line 3.** $= ${numTerm(BU, 2)} = ${BU.coef(2)} \\times ${pw(BU.p, 2)} \\times ${pw(BU.q, 3)} = ${e(BU.term(2))}$\n**Line 4.** $= ${v4(BU.term(2))}$ to 4 decimal places.`,
  },
  {
    type: "video",
    videoId: "9Y4bhtNSW8w",
    ...
    channel: "P McAleavey",
    why: "A Northern Ireland teacher takes Pascal's triangle, the expansion and binomial probability for CCEA Further Mathematics. Pause at each probability and check that its working has the four lines above: the labels, the term, the numbers in it, the rounding.",
  },
  {
    type: "gate",
    id: "g5",
  ...
  { type: "h", text: "5. At least: take away what you do not want" },
  {
    type: "p",
    md: "'At least 2' of Aoife's four throws means 2, 3 or 4 scores: three rows of the table. You could add those three. Because all five rows add up to 1, it is shorter to take away the two rows you do not want:\n$P(\\text{at least 2}) = 1 - \\left(P(0) + P(1)\\right)$\nKeep every figure your calculator shows until the last line, and round only there.",
  },
  {
    type: "figure",
    alt: `The free-throw table with the rows for 2, 3 and 4 scores shaded. Underneath, two routes to the same total: adding P(2) + P(3) + P(4), or taking P(0) + P(1) away from 1. Both give ${e(T.atLeast(2))}; the second is boxed because it uses fewer terms.`,
    svg: figLadderShaded,
    caption: "The shaded rows are 'at least 2'. The boxed route needs two terms instead of three.",
  },
  {
    type: "gate",
    id: "g6",
  ...
  { type: "h", text: "6. Write the bracket before the numbers" },
  {
    type: "p",
    md: `The whole of $P(0) + P(1)$ comes off the 1, so it goes inside a bracket. Leave the bracket out and only $P(0)$ is subtracted, while $P(1)$ is added on.\nFor Aoife that slip gives ${re("note#bracket", M.bracket)}, which cannot be a probability. The danger is when the slip gives a believable number instead.`,
  },
  {
    type: "figure",
    alt: `Two lines. The first, boxed: 1 − (P(0) + P(1)) = 1 − (${e(T.term(0))} + ${e(T.term(1))}) = ${e(T.atLeast(2))}. The second, dashed and struck through: 1 − P(0) + P(1) = 1 − ${e(T.term(0))} + ${e(T.term(1))} = ${re("note#bracket", M.bracket)}.`,
    svg: figBracket,
    caption: "Write '1 − (' first, then fill the bracket.",
  },
  {
  ...
  { type: "h", text: "7. At most, more than, fewer than" },
  {
    type: "p",
    md: "Every phrase names a set of whole numbers of successes, and the boundary is where marks go. 'At most 2' includes 2. 'More than 2' and 'fewer than 2' do not include it. Write the numbers out first, then take the shorter route: add those terms, or take all the others away from 1.",
  },
  {
    type: "figure",
    alt: "A table for 6 trials. Exactly 2 means 2 successes: P(2). At least 2 means 2, 3, 4, 5, 6: 1 − (P(0) + P(1)). At most 2 means 0, 1, 2: P(0) + P(1) + P(2). More than 2 means 3, 4, 5, 6: 1 − (P(0) + P(1) + P(2)). Fewer than 2 means 0, 1: P(0) + P(1).",
    svg: figPhrases,
    caption: "Six trials, and the number 2 in each phrase.",
  },
  {
  ...
  { type: "h", text: "8. Watch the terms build up" },
  {
    type: "p",
    md: "The PhET Plinko board drops balls through rows of pegs. At every peg a ball bounces right with a probability you choose, so each row of pegs is a trial and a right bounce is a success. The bin a ball lands in counts its right bounces, just as $r$ counts successes.",
  },
  plinkoBlock(
    "Open the Lab screen. Set 4 rows and a binary probability of 0.8, then drop at least 500 balls. Before you start, use the free-throw table at the top of this lesson to predict which two bins will fill most, and roughly what fraction of the balls each should hold.",
  ),
  {
    type: "gate",
    id: "g9",
  ...
  {
    type: "p",
    md: "Check the four conditions before you use the binomial model.\nName the success in the question's words, then write $p$, $q$ and $n$.\nWrite exactly $r$ as one term: the coefficient, $p^{r}$ and $q^{n-r}$.\nTurn at least, at most, more than and fewer than into a list of terms, bracketed when they come off 1.\nKeep every figure until the last line, then round as the question says.",
  },
  { type: "h", text: "In the exam" },
  {
    type: "p",
    md: "The probability parts follow Pascal's triangle and the expansion: usually exactly $r$ for about 3 marks, then at least $r$ for 3 more, each to 4 decimal places. The first marks are for the term written with its coefficient and powers; the last is for the rounded value. If you are stuck, write the label lines and the term with the right powers, because that line earns marks on its own.",
  },
...
const C = B.cafe;
const figGrid = pascalGridSvg({
  rows: ROWS,
  upTo: 5,
  title: "Pascal's triangle written out from row 0 to row 5, with an empty grid of seven boxes underneath for row 6",
});
questions.push(
  qq("0010", {
    style: "exam-style",
    ...
    figures: [figure(figGrid, "Pascal's triangle from row 0 (1) to row 5 (1 5 10 10 5 1), then a grid of seven empty boxes, numbered column 1 to column 7, for row 6.")],
    parts: [
      {
        id: "a",
...
const we1 = {
  ...
  figure: figure(figLabelBlank, "A blank label card with four lines to fill in: success, p, q and n."),
  steps: [
    {
      n: 1,
      working: `Late is the success: $p = ${BU.pStr}$, $q = 1 - ${BU.pStr} = ${BU.qStr}$, $n = ${BU.n}$`,
      decision: "Name the outcome the question counts before anything else. The 85% in the question is the probability of the other outcome, being on time, so it belongs to $q$.",
...
const sets = [
  {
    ...
    topic: TOPIC,
    kind: "interleaved",
    title: "From naming the success to at least r: short ladder",
    subject: "further-maths",
    units: ["FM3"],
    itemIds: [`dx.${TOPIC}.pre`, Q("0001"), Q("0002"), RP("01"), Q("0003"), Q("0004"), ftm2.id, Q("0005"), Q("0006"), ftm1.id],
...
const verification = [
  verLog(note.id, {
    ...
    "command-words": CMD,
    tariff: TAR,
    "maths-numeric": NUM("Every probability in the note, its gates and its six figures"),
