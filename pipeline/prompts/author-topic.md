...
- **A section is explain, then show, then check.** Explain: the idea in words, with its one honest "because" (a paragraph, or a titled `why`, `mustknow` or `examiner` callout). Show: the idea carried out in front of her, step by step: worked lines with numbers; in science, where there is often no arithmetic, a worked answer that applies the idea to a named case in numbered lines ("The figure's weeds as a full answer: 1. Some weeds had a mutation … [1] 2. The weedkiller killed …"; name the case, the figure or the marks, or set the situation up before the list) or questions about the case answered line by line; ordered steps under a "See it done" heading that names the case; a video or a sim. Step numbers may be written in any usual form ("1.", "1)", "1:", "Step 1:", "**Step 1: the equation.**", "Step one"), and working may sit in inline or display maths ($…$, $$…$$, \[…\]); a charge or an oxidation state on a species (Al³⁺, $\ce{O^{2-}}$, a 1− charge, the electrode's (−)) is part of the species, not arithmetic, so a line that only states charges is not a worked line. Not a show: a picture alone, a formula stated, a list of facts or definitions ("1. **Variation:** the individuals differ"), or a process told in general with no case: that is the explanation, and a concept section with only an explanation and a figure still needs a short See it applied to a case. Check: the gate. A check spends its demonstration: a second check later in the section needs its own worked lines after the first, unless the two checks follow one another directly. A section may run several cards of explanation and demonstration before its one check; a "See it done" section is the show step of the section above it, and a heading with no gate of its own does not close a section (its teaching runs on to the next gate), while a gate does: teaching that came before an earlier check is not counted again for the next one.
...
- **A worked example's own figure is seen with its steps hidden.** The app shows `we.figure` in the full example and again in the two faded versions and the problem version, where the later steps and the final answer are hers to write (the twin shows only `twin.figure`). So what those versions show must not print a value a faded version leaves to her (the weight 60 N she is about to calculate, the area under the graph, the optimum read off the curve) or the final answer. Keep the annotated figure in `figure` for the full example and give the worked example a `figurePlain`, the same drawing with those values left off: the faded and problem versions show `figurePlain` when it exists (25 Sep 2026), and the checks read it. figure-leaks.mjs lists every such value as a WE-LEAK line (a warning until the lead makes it fatal with `--we-fatal`); `npm run content:check` prints the same as a FIGURE warning.
